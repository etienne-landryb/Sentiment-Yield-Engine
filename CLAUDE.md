...
* Word cloud sources from GDELT `V2Themes`, not article text. Each theme field is `THEME_NAME,charoffset` — always split and take `SAFE_OFFSET(0)` before using the theme name (this was a real bug: unstripped offsets fragmented one theme into many spurious rows AND rendered as literal text in the cloud). Fixed at the BigQuery query level and defensively at read time. Themes are date-aware (`themes.parquet` = `[country_id, date, theme, count]`, top-20 per country/day via `fetch_themes_daily()`) — the word cloud slices to the selected period like sentiment/market, not always the whole snapshot. `build_country_from_snapshot` degrades gracefully (falls back to a whole-window total, or to nothing) if it ever sees a themes frame missing `date` or `country_id` — never crashes the whole panel over one malformed file.
...
* `load_country_bundles()` in `app/streamlit_app.py` is `@st.cache_data(ttl=3600)`. Streamlit's cache invalidation hashes that function's own code, not the code of everything it transitively calls (`pipeline.py`, etc.) — so a push that only changes `pipeline.py` can auto-redeploy successfully (new code on disk, new code rendering) while the cached bundle data itself still reflects the OLD `pipeline.py`, for up to an hour. Symptom: some UI changes visibly land (e.g. a Streamlit-layer conditional) while others don't (anything depending on the cached bundle's contents) — a genuinely confusing partial-looking deploy, not just "hasn't redeployed yet." A full "Reboot app" on Streamlit Cloud clears it; a plain git-push auto-redeploy may not.
* Any BigQuery ingest function (`gdelt_bq.py`) returns raw GDELT `country_code` (FIPS 10-4, e.g. `BE`) — the snapshot schema and everything downstream key on `country_id` (the app's own slug, e.g. `belgium`). The mapping is `{c["gdelt"]["source_country"]: c["id"] for c in settings.countries()}` (see `backfill.py`'s `_code_to_id()`). Skipping this and writing `country_code` straight to a snapshot file doesn't fail loudly at write time — it fails at READ time, in `pipeline.py`, with `KeyError: 'country_id'`, for every single country in the panel at once (not just the ones you were testing). Caught once already; always map before writing any new snapshot table.
* A local Streamlit run defaults to `DATA_MODE=demo` (see `settings.data_mode()`) unless `DATA_MODE=snapshot` is set in the shell/launcher — easy to forget when spinning up an ad hoc local preview, and the symptom isn't an error, just a much slower load (demo synthesizes every country's data from scratch) and `Resolved mode: demo` buried in the page text. Check that line before trusting any local snapshot-mode test.
* Never put `[theme]` keys in `.streamlit/config.toml` on Streamlit Community Cloud — defining any key under `[theme]` (even just `primaryColor`/`font`, no background/text colors) pins the app to a fixed "Custom Theme" and disables the native System/Light/Dark viewer switch entirely. Confirmed by diffing against three working sibling apps (DocViz, Dataset Fitness Check — no `config.toml` at all; Component X — `config.toml` with only `[client] toolbarMode = "auto"`, zero `[theme]` keys). If branding colors are wanted, apply them via the existing CSS-injection block (`_CSS` in `app/streamlit_app.py`) instead, styled directly with CSS selectors — never through `[theme]`. This cost multiple sessions to diagnose — don't reintroduce it.
...
python scripts/backfill.py --themes-only --months 6                         # themes only, keeps sentiment+market (no TWELVE_DATA_API_KEY needed)
